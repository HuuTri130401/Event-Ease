...
from app.schemas.sche_role import RoleReponse, RoleRequestCreate, RoleRequestUpdate
...
@router.put("{id}", response_model=DataResponse[RoleReponse], summary="Cập nhật role")
def update_role(id: int, role_data: RoleRequestUpdate, role_service: RoleService = Depends(get_role_service)) -> Any:
    try:
        role = role_service.update_role(role_data, id)
        return DataResponse().custom_response(code='200', message="Cập nhật role thành công", data=role)
    except Exception as e:
        raise CustomException(http_code=400, code='400', message=str(e))    
...
def delete_user(id: int, role_service: RoleService = Depends(get_role_service)):
    try:
        ...
        return DataResponse().custom_response(code='200', message=f"Xóa role thành công", data=result)
